...
llama_context::llama_context(
    ...
    // openwhisper local patch: disable Fused Gated Delta Net by default.
    //
    // The `auto_fgdn` device-placement check at `sched_reserve` (see
    // `llama_context::sched_reserve`, ~line 469) reserves a probe graph and
    // iterates its nodes looking for `GGML_OP_GATED_DELTA_NET` ops. The
    // assertion at line 487 (`strncmp(n->name, LLAMA_TENSOR_NAME_FGDN_AR
    // "-", prefix_len) == 0`) crashes the entire process during
    // `llama_new_context_with_model` for the Gemma 2 / Gemma 4 models we
    // actually load — the graph build for `LLM_ARCH_GEMMA4` (and Gemma 2)
    // never calls `ggml_gated_delta_net` itself, but at runtime a node ends
